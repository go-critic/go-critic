...
// with a definition from stdlib (stddef - definition from std).
//
// Rationale: readability.
type StddefChecker struct {
	ctx *Context

	// rootExpr holds the warning causing node.
	// It's required to be stored because we may do
	// recursive invocations of check method and loose it otherwise.
	rootExpr ast.Expr
...
// NewStddefChecker returns initialized checker for types and expressions.
func NewStddefChecker(ctx *Context) *StddefChecker {
...
// Check runs constexpr checks for f.
//
// Features
//
// Detects compile-time expressions that evaluate to some
// of the widely used Go stdlib named constants, like math.MaxInt32.
//
// TODO: variables, types.
// For example: func(http.ResponseWriter, *http.Request) => http.HandlerFunc.
func (c *StddefChecker) Check(f *ast.File) []Warning {
...
// checkExpr tries to recognize which named constant expr describes.
// Returns true if it found a match and produced a warning.
func (c *StddefChecker) checkExpr(expr ast.Expr) bool {
...
func (c *StddefChecker) checkCallExpr(call *ast.CallExpr) bool {
	if c.matchCallFunc(call, "unsafe", "Sizeof") {
		switch x := call.Args[0].(type) {
		case *ast.BasicLit:
			if x.Kind == token.INT {
				c.warn("math/bits.UintSize")
				return true
			}
		case *ast.CallExpr:
...
func (c *StddefChecker) warn(suggest string) {
	c.warnings = append(c.warnings, Warning{
		Node: c.rootExpr,
		Text: fmt.Sprintf("can replace %s with %s",
			nodeString(c.ctx.FileSet, c.rootExpr), suggest),
	})
}
